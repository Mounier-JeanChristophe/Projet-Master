...
set_attribute(m, "CPX_PARAM_EPINT", 1e-05) # https://www.ibm.com/docs/en/icos/22.1.1?topic=parameters-integrality-tolerance
...
# valeur absolue de (dti - dtm)
@variable(m, abs[i = 1:n])

# moyenne des distances
@variable(m, dtm)


# fonction objective: minimisation de la valeur absolue de la diffence 
@objective(m, Min, (1/n) * sum(abs[i] for i in 1:n))


# affectation des valeurs des dti
@constraint(m, [i=1:n], dt[i] == sum(2*d[i,j]*x[i,j,k] for j in 1:n, k in 1:r))

# affectation des valeurs des abs
@constraint(m, [i=1:n], abs[i] >= sum(2*d[i,j]*x[i,j,k] for j in 1:n, k in 1:r) - dtm)
@constraint(m, [i=1:n], abs[i] >= dtm - sum(2*d[i,j]*x[i,j,k] for j in 1:n, k in 1:r))
